...
Elm.HammingBit = Elm.HammingBit || {};
Elm.HammingBit.make = function (_elm) {
   "use strict";
   _elm.HammingBit = _elm.HammingBit || {};
   if (_elm.HammingBit.values)
   return _elm.HammingBit.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "HammingBit",
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Text = Elm.Text.make(_elm);
   var viewSmallNum = F4(function (w,
   h,
   bg,
   val) {
      ...
      w,
      h,
      $Graphics$Element.middle)($Graphics$Element.leftAligned($Text.style($JPCommon.codeStyle)($Text.fromString($Basics.toString(val))))));
   });
   var viewSmallBar = F2(function (w,
   bg) {
      return $Graphics$Element.color(bg)(A3($Graphics$Element.container,
      w,
      5,
      $Graphics$Element.middle)($Graphics$Element.leftAligned($Text.style($JPCommon.codeStyle)($Text.fromString("-")))));
   });
   var viewNum = F4(function (w,
   h,
   bg,
   val) {
      return $Graphics$Element.color(bg)(A3($Graphics$Element.container,
   ...
   var view = function (model) {
      return function () {
         var w = getWidth(model.place);
         var bg = _U.eq(model.valid,
         false) ? $Color.lightRed : _U.eq($List.tail(model.checkers),
         $Maybe.Just(_L.fromArray([]))) ? $Color.lightBlue : $Color.lightGreen;
         ...
         $JPCommon.lineHeight,
         bg),
         $List.reverse(model.checkers)))));
      }();
   };
   var init = F4(function (v,
   p,
   c,
   b) {
      return {_: {}
   ...
   _elm.HammingBit.values = {_op: _op
                            ,Model: Model
                            ,init: init
                            ,view: view
                            ,getWidth: getWidth
                            ,viewBar: viewBar
                            ,viewNum: viewNum
                            ,viewSmallBar: viewSmallBar
                            ,viewSmallNum: viewSmallNum};
   return _elm.HammingBit.values;
};
Elm.HammingCode = Elm.HammingCode || {};
Elm.HammingCode.make = function (_elm) {
   "use strict";
   _elm.HammingCode = _elm.HammingCode || {};
   if (_elm.HammingCode.values)
   return _elm.HammingCode.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "HammingCode",
   $Basics = Elm.Basics.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Graphics$Input = Elm.Graphics.Input.make(_elm),
   $HammingBit = Elm.HammingBit.make(_elm),
   $HammingCodeInput = Elm.HammingCodeInput.make(_elm),
   $JPCommon = Elm.JPCommon.make(_elm),
   $List = Elm.List.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm),
   $Text = Elm.Text.make(_elm);
   var incrementPowers = F2(function (val,
   old) {
      return _U.eq(val,
      0) ? old : function () {
         switch (old.ctor)
         {case "::": return _U.eq(old._0,
              val) ? A2(incrementPowers,
              val * 2,
              old._1) : A2($List._op["::"],
              val,
              A2($List._op["::"],
              ...
              old._1));
            case "[]":
            return _L.fromArray([val]);}
         _U.badCase($moduleName,
         "between lines 182 and 189");
      }();
   });
   var createPowerListsHelper = F2(function (counter,
   current) {
      return function () {
         switch (counter)
         {case 0:
            ...
            case 1:
            return _L.fromArray([current]);}
         return A2($List._op["::"],
         current,
         A2(createPowerListsHelper,
         counter - 1,
         A2(incrementPowers,1,current)));
      }();
   });
   var createPowerLists = function (len) {
      return A2(createPowerListsHelper,
      len,
      _L.fromArray([1]));
   };
   var filterPowerLists = F2(function (binary,
   powers) {
      return function () {
         switch (binary.ctor)
         {case "::": switch (binary._0)
              {case 0: return function () {
                      switch (powers.ctor)
                      {case "::":
                         return A2(filterPowerLists,
                           binary._1,
                           powers._1);
                         case "[]":
                         return _L.fromArray([]);}
            ...
            return _L.fromArray([]);}
         _U.badCase($moduleName,
         "between lines 154 and 161");
      }();
   });
   var countInList = F2(function (src,
   val) {
   ...
   var isPowerOfTwo = function (n) {
      return _U.cmp(n,
      1) < 1 ? true : _U.eq(A2($Basics._op["%"],
      n,
      2),
      0) ? isPowerOfTwo(n / 2 | 0) : false;
   };
   var addBlanks = F3(function (word,
   count,
   next) {
      return _U.eq(word,
      "") ? "" : _U.eq(count,
      ...
      word,
      count + 1,
      next * 2)) : A2($Basics._op["++"],
      A2($String.left,1,word),
      A3(addBlanks,
      A2($String.dropLeft,1,word),
      count + 1,
      next));
   });
   var encodedWord = F2(function (data,
   isEven) {
      return function () {
         var aug = $String.toList(A3(addBlanks,
         data,
         1,
         1));
         var size = $List.length(aug);
         var bits = A2($List.map,
         function (n) {
            return !_U.eq(n,
            _U.chr("1")) ? 0 : 1;
         },
         aug);
         var allPowers = $List.concat(A2(filterPowerLists,
         bits,
         createPowerLists(size)));
         var filledIn = A2($List.indexedMap,
         F2(function (n,c) {
            return _U.eq(c,
            _U.chr("_")) ? isEven ? A2($Basics._op["%"],
            A2(countInList,allPowers,n + 1),
            2) : 1 - A2($Basics._op["%"],
            A2(countInList,allPowers,n + 1),
            2) : !_U.eq(c,
            _U.chr("1")) ? 0 : 1;
   ...
   var decodedWord = F2(function (data,
   isEven) {
      return function () {
         var aug = $String.toList(data);
         var bits = A2($List.map,
         function (n) {
            return !_U.eq(n,
            _U.chr("1")) ? 0 : 1;
         ...
         aug);
         var targeted = A2($List.indexedMap,
         F2(function (n,c) {
            return isPowerOfTwo(n + 1) ? _U.chr("_") : c;
         }),
         aug);
         var reduced = A2($List.filter,
         function (c) {
            return !_U.eq(c,_U.chr("_"));
         },
         targeted);
         var correct = A2(encodedWord,
         $String.fromList(reduced),
         isEven);
         var difference = A3($List.map2,
         F2(function (a,b) {
            return _U.eq(a,b);
         }),
         correct,
         bits);
         var result = A3($List.map2,
         F2(function (v0,v1) {
            return {ctor: "_Tuple2"
                   ,_0: v0
                   ,_1: v1};
         ...
         bits,
         difference);
         var problemBit = A2($List.foldr,
         F2(function (_v13,old) {
            return function () {
               switch (_v13.ctor)
               {case "_Tuple2":
                  switch (_v13._1.ctor)
                    {case "_Tuple2":
                       return _U.eq(_v13._1._1,
                         false) ? old + _v13._0 : old;}
                    break;}
               _U.badCase($moduleName,
               "on line 119, column 48 to 83");
            }();
         }),
         0)(A3($List.map2,
         F2(function (v0,v1) {
         ...
         F2(function (n,_v19) {
            return function () {
               ...
               {case "_Tuple2":
                  return _U.eq(n + 1,
                    problemBit) ? {ctor: "_Tuple2"
                                  ,_0: _v19._0
                                  ,_1: false} : {ctor: "_Tuple2"
                                                ,_0: _v19._0
                                                ,_1: _v19._1};}
               _U.badCase($moduleName,
               "on line 120, column 38 to 88");
            }();
         }),
         result);
      }();
   ...
   model) {
      return function () {
         switch (action.ctor)
         {case "FlipParity":
            return _U.replace([["isEven"
                               ,$Basics.not(model.isEven)]],
              model);
            case "NoOp": return model;
            case "UpdateCode":
            return _U.replace([["codeword"
                               ,A2($HammingCodeInput.update,
                               action._0,
                               model.codeword)]],
              model);
            case "UpdateData":
            return _U.replace([["dataword"
                               ,A2($HammingCodeInput.update,
                               action._0,
                               model.dataword)]],
              model);}
   ...
   var UpdateCode = function (a) {
      return {ctor: "UpdateCode"
             ,_0: a};
   };
   var UpdateData = function (a) {
      return {ctor: "UpdateData"
             ,_0: a};
   };
   var FlipParity = {ctor: "FlipParity"};
   var NoOp = {ctor: "NoOp"};
   var updateMailbox = $Signal.mailbox(NoOp);
   var view = function (model) {
      return function () {
         var decoded = A2(decodedWord,
         model.codeword.content.string,
         model.isEven);
         var decodedOriginal = $Basics.fst($List.unzip($List.filter(function (p) {
            return _U.eq($Basics.snd(p),
            true);
         })($List.indexedMap(F2(function (i,
         n) {
            return isPowerOfTwo(i + 1) ? {ctor: "_Tuple2"
                                         ,_0: 0
                                         ,_1: false} : {ctor: "_Tuple2"
                                                       ,_0: $Basics.fst(n)
                                                       ,_1: true};
         ...
         function (_v26) {
            return function () {
               switch (_v26.ctor)
               {case "_Tuple2":
                  return _U.eq(_v26._1,
                    false) ? {ctor: "_Tuple2"
                             ,_0: 1 - _v26._0
                             ,_1: _v26._1} : {ctor: "_Tuple2"
                                             ,_0: _v26._0
                                             ,_1: _v26._1};}
               _U.badCase($moduleName,
               "on line 49, column 48 to 89");
            }();
         },
         decoded)))));
         var encoded = A2(encodedWord,
         model.dataword.content.string,
         model.isEven);
         return A2($Graphics$Element.flow,
         $Graphics$Element.down,
         _L.fromArray([A3($Graphics$Element.container,
                      310,
                      $JPCommon.titleHeight,
                      $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.titleStyle)($Text.fromString("Hamming Code"))))
                      ,A2($JPCommon.viewTableElement,
                      "Parity:",
                      A2($Graphics$Element.size,
                      80,
                      $JPCommon.lineHeight)(A2($Graphics$Input.button,
                      A2($Signal.message,
                      updateMailbox.address,
                      FlipParity),
                      model.isEven ? "Even" : "Odd")))
                      ,A3($Graphics$Element.container,
                      310,
                      $JPCommon.subtitleHeight,
                      $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.subtitleStyle)($Text.fromString("Encoder"))))
                      ,A2($Graphics$Element.flow,
                      $Graphics$Element.right,
                      _L.fromArray([A3($Graphics$Element.container,
                                   150,
                                   $JPCommon.lineHeight,
                                   $Graphics$Element.midRight)($Graphics$Element.leftAligned($Text.fromString("Data Word:")))
                                   ,$JPCommon.tableSpace
                                   ,A2($HammingCodeInput.view,
                                   A2($Signal.forwardTo,
                                   updateMailbox.address,
                                   UpdateData),
                                   model.dataword)]))
                      ,A2($JPCommon.viewTableElement,
                      "Encoding:",
                      $Graphics$Element.flow($Graphics$Element.right)($List.map($HammingBit.view)(A4($List.map3,
                      F3(function (i,v,p) {
                         return A4($HammingBit.init,
                         v,
                         i,
                         p,
                         true);
                      ...
                      $List.length(encoded)),
                      encoded,
                      createPowerLists($List.length(encoded))))))
                      ,A2($JPCommon.viewTableRow,
                      "Encoded Result:",
                      ...
                      310,
                      $JPCommon.subtitleHeight,
                      $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.subtitleStyle)($Text.fromString("Decoder"))))
                      ,A2($Graphics$Element.flow,
                      $Graphics$Element.right,
                      _L.fromArray([A3($Graphics$Element.container,
                                   150,
                                   $JPCommon.lineHeight,
                                   $Graphics$Element.midRight)($Graphics$Element.leftAligned($Text.fromString("Code Word:")))
                                   ,$JPCommon.tableSpace
                                   ,A2($HammingCodeInput.view,
                                   A2($Signal.forwardTo,
                                   updateMailbox.address,
                         ...
                         return function () {
                            ...
                            {case "_Tuple2":
                               return A4($HammingBit.init,
                                 ...
                                 _v30._1);}
                            _U.badCase($moduleName,
                            "on line 97, column 37 to 60");
                         }();
                      }),
                      _L.range(1,
                      $List.length(decoded)),
                      decoded,
                      createPowerLists($List.length(decoded))))))
                      ,A2($List.any,
                      function (_v34) {
                         return function () {
                            switch (_v34.ctor)
                            {case "_Tuple2":
                               return _U.eq(_v34._1,false);}
                            _U.badCase($moduleName,
                            "on line 101, column 31 to 41");
                         }();
                      },
                      decoded) ? A2($JPCommon.viewTableRow,
                      "Errors Detected!",
                      "Performing best-guess correction") : $Graphics$Element.empty
                      ,A2($JPCommon.viewTableRow,
                      "Decoded result:",
                      $String.fromList(A2($List.map,
                      function (c) {
   ...
   var init = {_: {}
              ,codeword: $HammingCodeInput.init
              ,dataword: $HammingCodeInput.init
              ,isEven: true};
   ...
   _elm.HammingCode.values = {_op: _op
                             ,Model: Model
                             ,init: init
                             ,NoOp: NoOp
                             ...
                             ,filterPowerLists: filterPowerLists
                             ,createPowerLists: createPowerLists
                             ,createPowerListsHelper: createPowerListsHelper
                             ,incrementPowers: incrementPowers
                             ,main: main
                             ,model: model
                             ,updateMailbox: updateMailbox};
   return _elm.HammingCode.values;
};
Elm.HammingCodeInput = Elm.HammingCodeInput || {};
Elm.HammingCodeInput.make = function (_elm) {
   "use strict";
   _elm.HammingCodeInput = _elm.HammingCodeInput || {};
   if (_elm.HammingCodeInput.values)
   return _elm.HammingCodeInput.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "HammingCodeInput",
   $Basics = Elm.Basics.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var isValid = function (content) {
      return _U.eq(content,
      $Graphics$Input$Field.noContent) ? true : A2($String.all,
      function (x) {
         return _U.eq(x,
         _U.chr("0")) || _U.eq(x,
         _U.chr("1"));
   ...
   model) {
      return function () {
         switch (action.ctor)
         {case "Update":
            return isValid(action._0) ? {_: {}
                                        ,content: action._0} : model;}
         _U.badCase($moduleName,
   ...
   var pushUpdate = F2(function (address,
   content) {
   ...
   model) {
      ...
      200,
      $JPCommon.lineHeight)(A4($Graphics$Input$Field.field,
      $JPCommon.inputStyle,
      pushUpdate(address),
      "1100101",
      model.content));
   ...
   _elm.HammingCodeInput.values = {_op: _op
                                  ,Model: Model
                                  ,init: init
                                  ,Update: Update
                                  ,update: update
