...
public class GameCode {
        ...
        for (int row = 1; row <= ROWS; row++) {
            ...
            for (int column = 2; column <= COLUMNS; column++) {
                if (destCol == column || grid[column][row] == 0) {
                    continue;
                } else if (grid[column][row] == grid[destCol][row] || grid[column][row] == -grid[destCol][row]) {
                    grid[destCol][row] = grid[destCol][row] + grid[column][row];
                    grid[column][row] = 0;
                    destCol++;
                } else {
     ...
     * 
     * @return False if new tile could not be added. True otherwise.
     */
    public boolean addNew2() {
        ...
        grid[col][row] = (random.nextBoolean()) ? 2 : 4;
        return true;
    }
