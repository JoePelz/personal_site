...
Elm.IPAddress.make = function (_elm) {
   "use strict";
   _elm.IPAddress = _elm.IPAddress || {};
   if (_elm.IPAddress.values)
   return _elm.IPAddress.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Color = Elm.Color.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $IPSegment = Elm.IPSegment.make(_elm),
   $JPCommon = Elm.JPCommon.make(_elm),
   $List = Elm.List.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm),
   $Text = Elm.Text.make(_elm);
   var toBinaryArray = F2(function (bits,
   value) {
      return _U.eq(bits,
   ...
   var bitSet = F2(function (n,
   mask) {
      return function () {
         var temp = A2($List.map,
         function (x) {
            return Math.pow(2,x);
         },
         _L.range(0,7));
         var source = A3($List.map2,
         F2(function (x,y) {
            return x * y;
         }),
         temp,
         $List.reverse(A2(toBinaryArray,
         8,
         n)));
         var powers = $List.reverse(A2($List.map,
         function (x) {
            return _U.cmp(x,
            mask) > -1 ? Math.pow(2,
            7 - x) : 0;
         ...
         A3($List.map2,
         F2(function (x,y) {
            return A2($Basics.max,x,y);
         }),
         powers,
         source));
      }();
   });
   var bitClear = F2(function (n,
   mask) {
      return function () {
         var powers = $List.reverse(A2($List.map,
         function (x) {
         ...
         powers,
         A2(toBinaryArray,8,n));
         return A3($List.foldr,
         F2(function (x,y) {
            return x + y;
         }),
         0,
         A2($List.take,mask,source));
      }();
   });
   var toBinary = F2(function (bits,
   value) {
      return _U.eq(bits,
      0) ? "" : function () {
      ...
      return function () {
         ...
         function () {
            ...
            {case 10: return "A";
               ...
               case 14: return "E";
               case 15: return "F";}
            return $Basics.toString(_v3);
         }());
      }();
   });
   var toStringIPH = F4(function (a,
   b,
   c,
   d) {
      return A2($Basics._op["++"],
   ...
   var viewAddress = F4(function (a,
   b,
   c,
   d) {
      return A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _L.fromArray([A2($JPCommon.viewTableRow,
                   "Dotted Decimal:",
                   A4(toStringIPD,a,b,c,d))
                   ,A2($JPCommon.viewTableRow,
                   "Binary:",
                   A4(toStringIPB,a,b,c,d))
                   ,A2($JPCommon.viewTableRow,
                   "Hex:",
                   A4(toStringIPH,a,b,c,d))]));
   });
   var viewRange = F5(function (a,
   b,
   c,
   d,
   mask) {
      return function () {
         var d$$ = A2(bitSet,
         d,
         A2($Basics.max,mask - 24,0));
         var c$$ = A2(bitSet,
         c,
         A2($Basics.max,mask - 16,0));
         var b$$ = A2(bitSet,
         b,
         A2($Basics.max,mask - 8,0));
         var a$$ = A2(bitSet,
         a,
         A2($Basics.max,mask,0));
         var d$ = A2(bitClear,
         d,
         A2($Basics.max,mask - 24,0));
         var c$ = A2(bitClear,
         ...
         _L.fromArray([A2($Graphics$Element.flow,
                      ...
                      _L.fromArray([A3($Graphics$Element.container,
                                   180,
                                   $JPCommon.lineHeight,
                                   $Graphics$Element.midRight)($Graphics$Element.leftAligned($Text.fromString("Network/Suffix:")))
                                   ,$JPCommon.tableSpace
                                   ,$Graphics$Element.leftAligned($Text.style($JPCommon.codeStyle)($Text.fromString(A2($Basics._op["++"],
                                   A3($String.slice,
                                   0,
                                   mask + (mask / 8 | 0),
                                   A4(toStringIPB,a,b,c,d)),
                                   "/"))))
                                   ,$Graphics$Element.color($Color.lightYellow)($Graphics$Element.leftAligned($Text.style($JPCommon.codeStyle)($Text.fromString(A3($String.slice,
                                   mask + (mask / 8 | 0),
                                   35,
                                   A4(toStringIPB,a,b,c,d))))))]))
                      ,A2($JPCommon.viewTableRow,
                      "Addresses in Range:",
                      $Basics.toString(Math.pow(2,
                      32 - mask)))
                      ,A3($Graphics$Element.container,
                      320,
                      $JPCommon.subtitleHeight,
                      $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.subtitleStyle)($Text.fromString("First Address"))))
                      ,A4(viewAddress,a$,b$,c$,d$)
                      ,A3($Graphics$Element.container,
                      320,
                      $JPCommon.subtitleHeight,
                      $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.subtitleStyle)($Text.fromString("Last Address"))))
                      ,A4(viewAddress,
                      a$$,
                      b$$,
                      c$$,
   ...
   model) {
      return function () {
         ...
         {case "NoOp": return model;
            case "Seg1Action":
            return _U.replace([["seg1"
                               ,A2($IPSegment.update,
                               action._0,
                               model.seg1)]],
              model);
            case "Seg2Action":
            return _U.replace([["seg2"
                               ,A2($IPSegment.update,
                               action._0,
                               model.seg2)]],
              model);
            case "Seg3Action":
            return _U.replace([["seg3"
                               ,A2($IPSegment.update,
                               action._0,
                               model.seg3)]],
              model);
            case "Seg4Action":
            return _U.replace([["seg4"
                               ,A2($IPSegment.update,
                               action._0,
                               model.seg4)]],
              model);
            case "SegMaskAction":
            return _U.replace([["segMask"
                               ,A2($IPSegment.update,
                               action._0,
                               model.segMask)]],
              model);}
   ...
   var SegMaskAction = function (a) {
      return {ctor: "SegMaskAction"
             ,_0: a};
   };
   var Seg4Action = function (a) {
      return {ctor: "Seg4Action"
             ,_0: a};
   };
   var Seg3Action = function (a) {
      return {ctor: "Seg3Action"
             ,_0: a};
   };
   var Seg2Action = function (a) {
      return {ctor: "Seg2Action"
             ,_0: a};
   };
   var Seg1Action = function (a) {
      return {ctor: "Seg1Action"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   ...
   var view = function (model) {
      return function () {
         var mask = model.segMask.value;
         var d = model.seg4.value;
         var c = model.seg3.value;
         var b = model.seg2.value;
         var a = model.seg1.value;
         return A2($Graphics$Element.flow,
         $Graphics$Element.down,
         _L.fromArray([A3($Graphics$Element.container,
                      310,
                      $JPCommon.titleHeight,
                      $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.titleStyle)($Text.fromString("IPv4 Network Address"))))
                      ,A3($Graphics$Element.container,
                      320,
                      50,
                      $Graphics$Element.middle)(A2($Graphics$Element.flow,
                      $Graphics$Element.right,
                      _L.fromArray([A2($IPSegment.view,
                                   A2($Signal.forwardTo,
                                   ...
                                   10,
                                   25,
                                   $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.codeStyle)($Text.fromString("."))))
                                   ,A2($IPSegment.view,
                                   A2($Signal.forwardTo,
                                   updateMailbox.address,
                                   Seg2Action),
                      ...
                      320,
                      $JPCommon.titleHeight,
                      $Graphics$Element.midBottom)($Graphics$Element.leftAligned($Text.style($JPCommon.titleStyle)($Text.fromString("Classless Addressing"))))
                      ...
                      _L.fromArray([A4($Graphics$Element.container,
                                   ...
                                   $Graphics$Element.leftAligned($Text.fromString("Prefix Length:")))
                                   ,$JPCommon.tableSpace
                                   ,A2($IPSegment.view,
                                   A2($Signal.forwardTo,
                                   updateMailbox.address,
   ...
   var init = {_: {}
              ,seg1: $IPSegment.init(255)
              ,seg2: $IPSegment.init(255)
              ,seg3: $IPSegment.init(255)
              ,seg4: $IPSegment.init(255)
              ,segMask: $IPSegment.init(32)};
   ...
   _elm.IPAddress.values = {_op: _op
                           ,Model: Model
                           ,init: init
                           ,NoOp: NoOp
                           ,Seg1Action: Seg1Action
                           ,Seg2Action: Seg2Action
                           ,Seg3Action: Seg3Action
                           ,Seg4Action: Seg4Action
                           ,SegMaskAction: SegMaskAction
                           ,update: update
                           ,updateMailbox: updateMailbox
                           ,view: view
                           ,viewAddress: viewAddress
                           ,viewRange: viewRange
                           ,toStringIPD: toStringIPD
                           ,toStringIPB: toStringIPB
                           ,toStringIPH: toStringIPH
                           ,toHex: toHex
                           ,toBinary: toBinary
...
Elm.IPSegment = Elm.IPSegment || {};
Elm.IPSegment.make = function (_elm) {
   "use strict";
   _elm.IPSegment = _elm.IPSegment || {};
   if (_elm.IPSegment.values)
   return _elm.IPSegment.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "IPSegment",
   $Basics = Elm.Basics.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var isValid = F2(function (cap,
   content) {
      return _U.eq(content,
      $Graphics$Input$Field.noContent) ? true : function () {
         var _v0 = $String.toInt(content.string);
         switch (_v0.ctor)
         {case "Err": return false;
            case "Ok": return _U.cmp(_v0._0,
              0) > -1 && _U.cmp(_v0._0,
              cap) < 1;}
         _U.badCase($moduleName,
   ...
   var getValue = function (content) {
      return _U.eq(content,
      $Graphics$Input$Field.noContent) ? 0 : function () {
         var _v3 = $String.toInt(content.string);
         switch (_v3.ctor)
         {case "Err": return 0;
         ...
         {case "Update":
            return A2(isValid,
              ...
              action._0) ? {_: {}
                           ,cap: model.cap
                           ,content: action._0
                           ,value: getValue(action._0)} : model;}
         _U.badCase($moduleName,
   ...
   model) {
      ...
      41,
      31)(A4($Graphics$Input$Field.field,
      $JPCommon.inputStyle,
      pushUpdate(address),
      "###",
      model.content));
