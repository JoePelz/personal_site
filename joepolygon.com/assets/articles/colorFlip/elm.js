...
Elm.PuzzleGame = Elm.PuzzleGame || {};
Elm.PuzzleGame.make = function (_elm) {
   "use strict";
   _elm.PuzzleGame = _elm.PuzzleGame || {};
   if (_elm.PuzzleGame.values)
   return _elm.PuzzleGame.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var cycleBox = F4(function (points,
   y,
   x,
   box) {
      return A2($List.any,
         ...
         return function () {
            ...
            {case "_Tuple2":
               return _U.eq(_v0._0,
                 x) && _U.eq(_v0._1,y);}
            _U.badCase($moduleName,
            "on line 79, column 31 to 43");
         }();
      },
      points) ? A2($Basics._op["%"],
      box + 1,
      3) : box;
   });
   var cycleRow = F3(function (points,
   y,
   row) {
      return A2($List.indexedMap,
      A2(cycleBox,points,y),
      row);
   });
   var cycleCoordinates = F2(function (points,
   grid) {
      return A2($List.indexedMap,
      cycleRow(points),
      grid);
   });
   var getNeighbors = F2(function (max,
   _v4) {
      return function () {
         switch (_v4.ctor)
         {case "_Tuple2":
            return _U.cmp(_v4._0,
              ...
              max) > -1 || (_U.cmp(_v4._0,
              0) < 0 || _U.cmp(_v4._1,
              0) < 0)) ? _L.fromArray([]) : function () {
                 var temp2 = $Debug.watch("Y: ")(_v4._1);
                 var temp = $Debug.watch("X: ")(_v4._0);
                 return _L.fromArray([{ctor: "_Tuple2"
                                      ,_0: _v4._0
                                      ,_1: _v4._1}
                                     ,{ctor: "_Tuple2"
   ...
   var NoOp = {ctor: "NoOp"};
   var updateChannel = $Signal.mailbox(NoOp);
   var Click = function (a) {
      return {ctor: "Click",_0: a};
   };
   var inputs = $Signal.mergeMany(_L.fromArray([$Signal.map(Click)($Signal.sampleOn($Mouse.clicks)(A3($Signal.map2,
                                               F2(function (v0,v1) {
                                                  return {ctor: "_Tuple2"
                                                         ,_0: v0
                                                         ,_1: v1};
   ...
   var collageHeight = 400.0;
   var collageWidth = 400.0;
   var viewBox = F4(function (i,
   range,
   y,
   value) {
      return function () {
         var bg = _U.eq(value,
         0) ? $Color.yellow : _U.eq(value,
         1) ? $Color.blue : $Color.red;
         var h = collageHeight / $Basics.toFloat(range) - 2;
         var w = collageWidth / $Basics.toFloat(range) - 2;
         var x = $Basics.toFloat(i) / $Basics.toFloat(range) * collageWidth - collageWidth / 2 + collageWidth / ($Basics.toFloat(range) * 2);
         return $Graphics$Collage.move({ctor: "_Tuple2"
                                       ,_0: x
                                       ,_1: y})($Graphics$Collage.filled(bg)(A2($Graphics$Collage.rect,
         w,
         h)));
      }();
   });
   var viewRow = F3(function (i,
   range,
   row) {
      return function () {
         var y = $Basics.toFloat(i) / $Basics.toFloat(range) * collageHeight - collageHeight / 2 + collageHeight / ($Basics.toFloat(range) * 2);
         return $Graphics$Collage.group(A2($List.indexedMap,
         F2(function (i,e) {
            return A4(viewBox,
            i,
            range,
   ...
   var view = F2(function (width,
   model) {
      return function () {
         var height = $JPCommon.titleHeight + $JPCommon.subtitleHeight + $Basics.floor(collageHeight) + 20 + $JPCommon.subtitleHeight;
         ...
         _L.fromArray([A3($Graphics$Element.container,
                      $Basics.floor(collageWidth),
                      $JPCommon.titleHeight,
                      $Graphics$Element.middle)($Graphics$Element.leftAligned($Text.style($JPCommon.titleStyle)($Text.fromString("Puzzle Game"))))
                      ,A3($Graphics$Element.container,
                      $Basics.floor(collageWidth),
                      $JPCommon.subtitleHeight,
                      $Graphics$Element.middle)($Graphics$Element.leftAligned($Text.style($JPCommon.subtitleStyle)($Text.fromString("Turn all the tiles blue!"))))
                      ,A3($Graphics$Collage.collage,
                      $Basics.floor(collageWidth),
                      $Basics.floor(collageHeight),
                      A2($List._op["::"],
                      ...
                      120,
                      35)(A2($Graphics$Input.button,
                      A2($Signal.message,
                      updateChannel.address,
                      Reset(5)),
                      "Reset")))])));
      }();
   });
   ...
   model) {
      return function () {
         var tAction = $Debug.watch("action")(action);
         return function () {
            switch (action.ctor)
            {case "Click":
               switch (action._0.ctor)
                 {case "_Tuple2":
                    switch (action._0._0.ctor)
                      {case "_Tuple2":
                         return function () {
                              var adjustedY = action._0._0._1 - $JPCommon.titleHeight - $JPCommon.subtitleHeight;
                              var yi = _U.cmp(adjustedY,
                              0) < 0 ? -1 : adjustedY / ($Basics.floor(collageHeight) / model.size | 0) | 0;
                              var adjustedX = action._0._0._0 - (action._0._1 / 2 | 0) + $Basics.floor(collageWidth / 2);
                              var xi = _U.cmp(adjustedX,
                              0) < 0 ? -1 : adjustedX / ($Basics.floor(collageWidth) / model.size | 0) | 0;
   ...
   _elm.PuzzleGame.values = {_op: _op
                            ,Model: Model
                            ,init: init
                            ,getRows: getRows
                            ...
                            ,cycleCoordinates: cycleCoordinates
                            ,cycleRow: cycleRow
                            ,cycleBox: cycleBox
                            ,view: view
                            ,viewRow: viewRow
